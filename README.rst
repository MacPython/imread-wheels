####################################
Building and uploading imread wheels
####################################
...
* builds a imread wheel, linking against these fresh builds;
...
* making a commit to the `imread-wheels` repository (e.g. with `git
...
Which imread commit does the repository build?
==============================================

The `imread-wheels` repository will build the commit specified in the
...
be something like::
    ...
    wheel-uploader -r warehouse -u $CDN_URL -s -v -w ~/wheelhouse -t macosx imread $VERSION
    wheel-uploader -r warehouse -u $CDN_URL -s -v -w ~/wheelhouse -t manylinux1 imread $VERSION
...
``imread`` is the root name of the wheel(s) to download / upload, and
``0.6`` is the version to download / upload.
...
``imread-0.6-`` from http://wheels.scipy.org to ``~/wheelhouse``,
